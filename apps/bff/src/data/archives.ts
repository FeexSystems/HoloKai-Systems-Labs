...
export const mockArchives = CIVILIZATIONS.flatMap(civ => {
  const base = { civilizationId: civ.id, era: civ.era, region: civ.region };
  const records = [];
  
  records.push({ id: uuidv4(), title: civ.name, category: 'civilization', description: civ.description, ...base });
  civ.people.forEach(p => records.push({ id: uuidv4(), title: p.name, category: 'person', description: p.desc, ...base }));
  civ.places.forEach(p => records.push({ id: uuidv4(), title: p.name, category: 'place', description: p.desc, ...base }));
  civ.events.forEach(e => records.push({ id: uuidv4(), title: e.name, category: 'event', description: e.desc, era: e.era, region: civ.region }));
  civ.concepts.forEach(c => records.push({ id: uuidv4(), title: c.name, category: 'concept', description: c.desc, ...base }));
  civ.artifacts.forEach(a => records.push({ id: uuidv4(), title: a.name, category: 'artifact', description: a.desc, ...base }));
