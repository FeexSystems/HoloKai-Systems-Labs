...
import { SplineStage, isWebGLAvailable } from "@landing/components/lab/SplineStage";
...
const SPLINE_SCENE =
  import.meta.env.VITE_SPLINE_SCENE_URL?.trim() || "https://prod.spline.design/UxqPsHJtrHQardbV/scene.splinecode";
const CORE_URL = import.meta.env.VITE_CORE_URL?.trim() || "/";
...
export default function SplineLab() {
  ...
  return (
    <main className="landing-theme fixed inset-0 flex flex-col bg-[#020202] text-zinc-100 selection:bg-amber-500 selection:text-black">
      <header className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-4 md:p-6">
        ...
        <div className="pointer-events-auto flex flex-wrap items-center justify-end gap-2">
          ...
          <Link
            to="/oracle-portal"
            className="flex items-center gap-2 border border-amber-500/40 bg-amber-500/20 px-4 py-2 text-[9px] font-bold tracking-[0.18em] text-amber-300 backdrop-blur-xl transition hover:bg-amber-500/35 hover:text-white"
          >
            <Sparkles className="h-3.5 w-3.5 text-amber-400" />
            HOLOKAI ORACLE PORTAL
          </Link>
          <Link
            to="/core"
