 ...
 */
export default function AmbientSoundscapeToggle({ activeText = '' }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end">
      ...
      {isMenuOpen && (
        <div className="mb-3 w-72 sm:w-80 rounded-2xl border border-amber-500/30 bg-zinc-950/95 p-4 shadow-2xl backdrop-blur-xl animate-fadeIn space-y-4 text-zinc-100">
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <button
              ...
              className={`w-8 h-4 rounded-full transition-colors relative ${
                autoDetect ? 'bg-amber-500' : 'bg-zinc-800'
              }`}
            >
              <div
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="p-2.5 rounded-full border border-white/10 bg-zinc-950/90 text-zinc-300 hover:text-white hover:border-amber-500/40 shadow-xl backdrop-blur-md transition-all"
        ...
        <button
          ...
          className={`group px-3.5 py-2.5 rounded-full border transition-all shadow-[0_0_25px_rgba(0,0,0,0.8)] backdrop-blur-md flex items-center gap-2.5 font-mono text-xs ${
            isPlaying
              ? 'border-amber-500/60 bg-zinc-950/95 text-amber-300 shadow-[0_0_20px_rgba(245,158,11,0.3)]'
              : 'border-white/10 bg-zinc-950/80 text-zinc-400 hover:text-zinc-200 hover:border-amber-500/30'
          }`}
          ...
          {isPlaying ? (
            <>
              <div className="relative flex items-center justify-center">
                ...
                <Volume2 className="w-4 h-4 text-amber-400 relative z-10" />
              </div>
              ...
              <div className="flex items-end gap-0.5 h-3">
                ...
                <span className="w-0.5 h-2/3 bg-amber-400 rounded-full animate-pulse" style={{ animationDelay: '200ms' }} />
                <span className="w-0.5 h-4/5 bg-amber-400 rounded-full animate-pulse" style={{ animationDelay: '400ms' }} />
              </div>
            </>
          ) : (
            <>
              <VolumeX className="w-4 h-4 text-zinc-500 group-hover:text-amber-400 transition-colors" />
