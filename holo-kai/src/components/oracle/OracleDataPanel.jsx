...
const HISTORICAL_PREVIEW_ENTRIES = [
  {
...
export default function OracleDataPanel({ className = '', onSelectCivilization, activePeriod }) {
  const { toast } = useToast();
  const [selectedRegion, setSelectedRegion] = useState('all');
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [confidenceScore, setConfidenceScore] = useState(96.4);
  const [expandedCardId, setExpandedCardId] = useState(null);
  const [activeNarratingId, setActiveNarratingId] = useState(null);
  ...
  const handleDownloadCSV = () => {
    ...
    const csvRows = [
      ['Digitized Manuscripts', displayManuscripts, activePeriod ? activePeriod.title : "1500 CE", selectedRegion, "ZERO_HALLUCINATION_GROUNDED"],
      ['Griot Oral Archives', displayOralArchives, activePeriod ? activePeriod.title : "1500 CE", selectedRegion, "ZERO_HALLUCINATION_GROUNDED"],
      ['Civilization Graph Nodes', displayGraphNodes, activePeriod ? activePeriod.title : "1500 CE", selectedRegion, "ZERO_HALLUCINATION_GROUNDED"],
    ...
    HISTORICAL_ACTIVITY_DATA.forEach(item => {
      csvRows.push([`Activity (${item.year})`, `Manuscripts: ${item.manuscripts}, Archives: ${item.archives}`, item.era, selectedRegion, 'VERIFIED']);
    });
    ...
    toast({
      ...
      className: "glass-panel border-amber-500/40 bg-zinc-950/95 text-amber-300 font-mono text-xs",
    });
  };
  ...
  const handleDownloadReport = () => {
    ...
    // Trigger Toast Notification using Toaster component with Download CSV and Copy Link actions
    toast({
      title: "Oracle Report Exported",
      description: `JSON & CSV telemetry data for ${activePeriod ? activePeriod.label : '1500 CE'} (${selectedRegion.toUpperCase()}) compiled successfully.`,
      className: "glass-panel border-amber-500/40 bg-zinc-950/95 text-white shadow-[0_0_20px_rgba(232,184,75,0.3)] font-mono",
      action: (
        <div className="flex items-center gap-1.5 shrink-0 ml-2">
          <button
            onClick={() => handleDownloadCSV()}
            className="px-2.5 py-1 rounded bg-amber-500/20 hover:bg-amber-500/40 text-amber-300 border border-amber-500/40 text-xs font-mono flex items-center gap-1 transition-colors"
          >
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(jsonStringContent);
              toast({
                title: "JSON Copied to Clipboard!",
                description: "Full telemetry payload copied.",
                className: "glass-panel border-emerald-500/40 text-emerald-300 bg-zinc-950/95 font-mono text-xs",
              });
            }}
            className="px-2.5 py-1 rounded bg-amber-500/20 hover:bg-amber-500/40 text-amber-300 border border-amber-500/40 text-xs font-mono flex items-center gap-1 transition-colors"
          >
  ...
  const handlePlayNarrator = (entry) => {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-panel p-4 rounded-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleDownloadCSV}
            onMouseEnter={() => retroAudio.playGlassHoverHum()}
            className="glass-panel border border-amber-400/40 text-amber-300 hover:text-white text-xs font-mono px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-amber-500/20 transition-all cursor-pointer"
          ...
          <button
            ...
            onMouseEnter={() => retroAudio.playGlassHoverHum()}
            className="glass-panel shadow-[0_0_15px_rgba(232,184,75,0.25)] hover:shadow-[0_0_25px_rgba(232,184,75,0.45)] border border-amber-400/40 text-amber-300 hover:text-white text-xs font-mono px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-amber-500/20 transition-all cursor-pointer"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          onMouseEnter={() => retroAudio.playGlassHoverHum()}
          className="glass-panel p-5 rounded-xl flex flex-col justify-between relative overflow-hidden group transition-all duration-300"
        >
          ...
          <div className="mt-4 pt-3 border-t border-amber-500/10 text-[11px] text-zinc-400 flex justify-between items-center">
          ...
          {HISTORICAL_PREVIEW_ENTRIES.map((entry) => {
            ...
            return (
              <div
                ...
                className={`glass-panel p-5 rounded-xl border transition-all duration-300 relative group cursor-pointer ${
                  isExpanded
              ...
              >
                ...
                {isExpanded && (
                  <div className="mt-4 pt-3 border-t border-amber-500/20 space-y-2 animate-fadeIn">
