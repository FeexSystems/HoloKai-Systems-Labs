...
export default function OracleTimelineScrubber({
  ...
  const toggleAutoPlay = () => {
    ...
    if (isPlaying) {
      ...
      const interval = setInterval(() => {
        onPeriodSelect((prevPeriod) => {
          const prevId = prevPeriod?.id || activePeriodId;
          const currIdx = HISTORICAL_PERIODS.findIndex((p) => p.id === prevId);
          const nextIdx = (currIdx + 1) % HISTORICAL_PERIODS.length;
          retroAudio.playOracleChime();
          return HISTORICAL_PERIODS[nextIdx];
        });
      }, 3500);

      // Save interval reference to clear on stop
      window.__timelineInterval = interval;
    }
  };

  React.useEffect(() => {
    if (!isPlaying && window.__timelineInterval) {
      clearInterval(window.__timelineInterval);
      window.__timelineInterval = null;
    }
    return () => {
      if (window.__timelineInterval) {
        clearInterval(window.__timelineInterval);
      }
    };
  }, [isPlaying]);
