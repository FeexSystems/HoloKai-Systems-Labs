...
function CollectiveVideoCard({
  ...
  return (
    <article className="group overflow-hidden border border-[var(--color-border)] bg-background">
      <div className="relative aspect-video bg-black">
        ...
        >
          <span
            className={`grid h-16 w-16 place-items-center rounded-full border border-brand/50 bg-black/55 text-brand backdrop-blur-md transition ${
        ...
        <div className="absolute bottom-4 left-4 right-4 z-10 flex items-end justify-between gap-3">
          <div>
            <p className="flex items-center gap-2 text-[9px] font-bold tracking-[0.28em] text-brand">
              <Film className="h-3 w-3" />
              CINEMA FEED
            </p>
            <h3 className="mt-1 font-display text-xl font-bold text-foreground sm:text-2xl">{title}</h3>
          </div>
          <span className="border border-white/15 bg-black/50 px-2 py-1 text-[9px] tracking-wider text-muted">
            {playing ? "PLAYING" : "MP4"}
          </span>
        </div>
      </div>
      <p className="border-t border-border-subtle px-5 py-4 text-sm leading-relaxed text-muted">{caption}</p>
    </article>
  );
}
...
export function MeetTheVanguard() {
  ...
  return (
    <section
      id="collective"
      className="relative z-10 border-t border-border-subtle bg-[#050505] py-24 md:py-32"
    >
      <div className="mx-auto max-w-[1600px] px-6">
        <div className="max-w-3xl">
          ...
          <h2 className="font-display text-5xl font-light leading-none tracking-tight sm:text-7xl">
            Meet the{" "}
            <span className="font-bold italic text-brand">Vanguard.</span>
          </h2>
        ...
        {/* Videos */}
        <div className="mt-14">
          <p className="mb-5 flex items-center gap-2 text-[10px] font-bold tracking-[0.3em] text-muted">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {COLLECTIVE_IMAGES.map((image) => (
              <button
                ...
                className="group relative overflow-hidden border border-[var(--color-border)] bg-background text-left"
              >
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <p className="mt-2 max-w-md text-sm text-muted">{image.caption}</p>
                  </div>
                </div>
              </button>
            ))}
      ...
      {lightbox && (
        ...
        >
          <button
            type="button"
            className="absolute right-5 top-5 border border-white/20 px-4 py-2 text-[10px] tracking-[0.2em] text-muted hover:text-foreground"
            onClick={() => setLightbox(null)}
          >
            CLOSE
          </button>
