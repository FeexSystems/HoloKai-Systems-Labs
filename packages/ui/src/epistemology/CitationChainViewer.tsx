        ...
        {citations.map((cite, idx) => (
          <div key={cite.id} className="flex flex-col relative group">
            {idx !== citations.length - 1 && (
              <div className="absolute left-[11px] top-7 bottom-[-16px] w-[2px] bg-zinc-800 group-hover:bg-amber-500/30 transition-colors" />
            )}
            <button 
            ...
            >
              <div className="w-6 h-6 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center shrink-0 mt-0.5 z-10 group-hover:border-amber-500/50 group-hover:bg-amber-500/10 transition-colors">
                <span className="text-[10px] font-mono text-zinc-400 group-hover:text-amber-400">{idx + 1}</span>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <ArrowRight className="w-3 h-3 text-zinc-600 group-hover:text-amber-500 transition-colors opacity-0 group-hover:opacity-100" />
                </div>
              </div>
            </button>
          </div>
