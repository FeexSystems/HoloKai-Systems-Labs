...
export function EvidenceMatrix({
  ...
  return (
    <div
      className={`rounded-2xl border border-[var(--color-border)] bg-[#12121a] p-6 md:p-8 space-y-6 text-white backdrop-blur-xl ${className}`}
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div>
          <span className="text-[10px] font-mono uppercase tracking-widest text-[var(--color-brand)] font-bold">
      ...
      <div className="p-4 rounded-xl bg-[#0a0a0f] border border-white/5 space-y-1">
        ...
        <p className="text-sm font-medium text-zinc-100 leading-relaxed">"{claim}"</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3">
          {sampleEvidence.map((span) => (
            <div
              key={span.id}
              className="p-4 rounded-xl bg-black/40 border border-white/5 space-y-2 hover:border-[var(--color-border)] transition-colors"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[var(--color-brand)]">{span.sourceTitle}</span>
      ...
      {citations.length > 0 && (
        <div className="pt-4 border-t border-white/5 flex flex-wrap items-center gap-2 text-xs font-mono text-zinc-400">
          <span className="text-[var(--color-brand)] font-bold">Citation Index:</span>
          {citations.map((c, i) => (
