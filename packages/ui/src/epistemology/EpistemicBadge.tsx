...
export function EpistemicBadge({
  ...
  return (
    <div className="relative inline-block">
      ...
      {showTooltip && hovered && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 rounded-xl bg-[#08110C] border border-[#8FAF91]/30 text-white text-xs z-50 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150">
          <div className="font-mono text-[10px] uppercase font-bold text-[#A9D5B0] mb-1">
            Epistemic Definition
          </div>
          <p className="text-[#8C9991] text-[11px] leading-relaxed">{token.description}</p>
          <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-[#08110C]" />
        </div>
      )}
    </div>
  );
