...
export const CULTURAL_GLOSSARY_DICTIONARY: Record<string, GlossaryTerm> = {
  kandake: {
    term: 'Kandake',
...
export function CulturalGlossary({ soundEffectsEnabled = true }: { soundEffectsEnabled?: boolean }) {
  const [activeTerm, setActiveTerm] = useState<GlossaryTerm | null>(null);
  const [position, setPosition] = useState({ top: 0, left: 0 });
  const [customQuery, setCustomQuery] = useState<CustomQuery | null>(null);
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-[320px] rounded-2xl shadow-2xl border p-4 relative overflow-hidden backdrop-blur-xl bg-zinc-950/95 border-[var(--color-border)] text-zinc-100 shadow-[0_0_20px_rgba(0,0,0,0.9)]"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b pb-2.5 mb-3 border-[var(--color-border)]">
            <div className="flex items-center gap-2">
              <div className="p-1.5 rounded-lg bg-[var(--color-surface-hover)] text-[var(--color-brand)]">
                <BookOpen className="w-4 h-4" />
              </div>
              <div>
                <span className="text-[10px] font-mono tracking-widest uppercase font-bold text-[var(--color-brand)] flex items-center gap-1">
                  Cultural Glossary
          ...
          {customQuery && !activeTerm && (
            <div className="space-y-3">
              <p className="text-xs">
                Highlighted: <strong className="text-[var(--color-brand)] font-mono">"{customQuery.term}"</strong>
              </p>
              <button
                onClick={handleFetchDynamicDefinition}
                disabled={isDynamicLoading}
                className="w-full py-2 px-3 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition-all bg-[var(--color-brand)] hover:bg-[var(--color-brand)] text-zinc-950 font-bold"
              >
                {isDynamicLoading ? (
                  <>
          ...
          {activeTerm && (
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h4 className="text-sm font-bold font-mono text-[var(--color-brand)] flex items-center gap-2">
                    {activeTerm.term}
                    {activeTerm.pronunciation && (
                      <button
                        onClick={() => speakTerm(activeTerm.term)}
                        className="p-1 hover:text-[var(--color-brand)] transition-colors"
                ...
                {activeTerm.region && (
                  <span className="text-[9px] font-mono px-2 py-0.5 rounded-full border shrink-0 bg-[var(--color-surface-hover)] border-[var(--color-border)] text-[var(--color-brand)]">
                    {activeTerm.region}
                  </span>
                )}
              </div>
              ...
              {activeTerm.culturalContext && (
                <div className="p-2.5 rounded-xl text-[11px] leading-normal border bg-zinc-900/80 border-white/5 text-zinc-300">
                  <strong className="text-[var(--color-brand)] font-mono uppercase text-[9px] block mb-0.5">Cultural Significance:</strong>
                  {activeTerm.culturalContext}
                </div>
              )}
            </div>
