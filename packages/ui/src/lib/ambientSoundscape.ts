...
export const ERA_SOUNDSCAPES: EraSoundscape[] = [
  {
    id: 'kemet',
...
class AmbientSoundscapeEngine {
  ...
  buildSynthGraph(config: EraSoundscape) {
    this.stopSynthNodes();
  ...
  createAtmosphericNoise(now: number) {
    if (!this.audioCtx || !this.masterGain) return;
  ...
  transitionToEra(config: EraSoundscape) {
    if (!this.audioCtx) return;
    const now = this.audioCtx.currentTime;
