...
class HoloKaiOracleVoiceEngine {
  ...
  private agentConfig = DEEPGRAM_AGENT_CONFIG.agent;
  private audioCache: Map<string, string> = new Map();
  private cloudOnlyMode: boolean = true;

  setupAudioAnalyser(audioElement: HTMLAudioElement) {
  ...
  getFrequencyData(arraySize: number = 32): Uint8Array {
  ...
  async speakResponse(data: any, options: any = {}): Promise<void> {
    const textToSpeak =
      typeof data === 'string'
        ? data
    ...
    if (audioUrl) {
      ...
      console.warn('Deepgram Cloud Voice unavailable. Speech synthesis skipped.');
      return Promise.resolve();
    }
  }

  executeArithmetic(operation: string, numbers: string[]): string {
