...
export function DomainSearch({
  ...
  return (
    <div
      className={`rounded-[24px] border border-[var(--color-border)] bg-[#0a0a0f]/90 p-3 shadow-2xl backdrop-blur-2xl transition-all duration-300 hover:border-[var(--color-border)] ${className}`}
    >
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-3">
        <div className="flex-1 flex items-center gap-3 px-4 w-full">
          ...
          <span className="text-[var(--color-brand)] font-mono text-lg" aria-hidden="true">🔍</span>
          <input
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto px-2">
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-mono font-bold border transition-all ${
              beastMode
                ? 'bg-[var(--color-surface-hover)] border-[var(--color-brand)] text-[var(--color-brand)] shadow-md shadow-glow-brand'
          ...
          <button
            type="submit"
            className="h-12 rounded-[18px] px-8 bg-gradient-to-r from-[var(--color-brand)] to-[var(--color-brand-contrast)] text-black font-extrabold text-sm hover:brightness-110 shadow-glow-brand transition-all"
          >
            Search OS →
          </button>
        </div>
