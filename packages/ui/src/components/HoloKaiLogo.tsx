...
export function HoloKaiLogo({
  ...
  if (imgError) {
    return (
      <div className={`inline-flex items-center gap-2 select-none font-display shrink-0 ${className || ""}`.trim()}>
        <span className="flex h-6 w-6 items-center justify-center rounded-sm bg-surface-hover border border-border text-brand font-bold text-[10px] tracking-widest shadow-[0_0_8px_rgba(245,158,11,0.3)] shrink-0">
          HK
        </span>
        {variant !== "icon" && (
          <span className="text-sm font-bold tracking-widest text-white uppercase whitespace-nowrap">
            Holo<span className="text-brand">Kai</span>
          </span>
        )}
      </div>
    );
  ...
  return (
    <img
      ...
      className={`select-none object-contain shrink-0 max-h-full max-w-full inline-block ${className || ""}`.trim()}
      decoding="async"
      loading="eager"
    />
  );
