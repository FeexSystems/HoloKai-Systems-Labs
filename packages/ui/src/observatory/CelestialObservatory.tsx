...
export default function CelestialObservatory() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-6 space-y-6 text-zinc-100 font-sans">
      {/* HERO BANNER */}
      <div className="relative rounded-3xl border border-[var(--color-border)] bg-gradient-to-r from-zinc-950 via-zinc-900 to-[var(--pui-forest-deep)]/40 p-6 sm:p-8 shadow-2xl overflow-hidden backdrop-blur-xl">
        <div className="absolute top-0 right-0 w-80 h-80 bg-[var(--color-surface-hover)] rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--color-border)] bg-[var(--color-surface-hover)] text-[var(--color-brand)] text-xs font-mono font-bold tracking-wider uppercase">
              <Star className="w-3.5 h-3.5 text-[var(--color-brand)] fill-amber-400" />
          ...
          <div className="flex items-center gap-3">
            <div className="px-4 py-2 rounded-2xl bg-zinc-900 border border-[var(--color-border)] text-xs font-mono text-[var(--color-brand)] flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-3">
          <h3 className="text-xs font-mono font-bold text-[var(--color-brand)] uppercase tracking-wider flex items-center gap-2">
              ...
              <div
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer space-y-2 ${
                  selectedSite.id === site.id
                    ? 'border-[var(--color-border)] bg-[var(--color-brand)]/15 shadow-lg'
                    : 'border-white/10 bg-zinc-950/80 hover:border-[var(--color-border)] hover:bg-zinc-900'
                }`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-[var(--color-brand)] font-bold uppercase">
                    {site.era}
                  </span>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="p-6 rounded-3xl border border-[var(--color-border)] bg-zinc-950/90 shadow-2xl space-y-6">
                ...
                {['Akhet (Inundation)', 'Peret (Emergence)', 'Shemu (Harvest)'].map((season) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-[10px] font-mono transition ${
                      activeSeason === season
                        ? 'bg-[var(--color-brand)] text-zinc-950 font-bold'
            ...
            <div className="relative w-full h-64 rounded-2xl border border-white/10 bg-gradient-to-b from-zinc-950 via-zinc-900 to-[var(--pui-forest-deep)]/30 overflow-hidden flex items-center justify-center p-4">
              ...
              <svg
                ...
                viewBox="0 0 300 200"
              >
                <circle cx="150" cy="100" r="70" fill="none" stroke="#A9D5B0" strokeWidth="1" strokeDasharray="4 4" />
                <circle cx="150" cy="100" r="45" fill="none" stroke="#10B981" strokeWidth="1" strokeDasharray="2 2" />

                {/* Stars */}
                <circle cx="150" cy="30" r="4" fill="#A9D5B0" className="animate-pulse" />
                <circle cx="215" cy="100" r="3" fill="#3B82F6" />
                <circle cx="105" cy="145" r="3" fill="#EC4899" />
                <circle cx="150" cy="100" r="6" fill="#FCD34D" />

                <line x1="150" y1="30" x2="215" y2="100" stroke="#A9D5B0" strokeWidth="0.5" strokeOpacity="0.5" />
                <line x1="215" y1="100" x2="105" y2="145" stroke="#A9D5B0" strokeWidth="0.5" strokeOpacity="0.5" />
              </svg>

              <div className="absolute bottom-3 left-3 px-3 py-1 rounded-lg bg-zinc-950/80 border border-white/10 backdrop-blur-md text-xs font-mono text-[var(--color-brand)]">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-mono text-zinc-300">
                ...
                <span className="text-[var(--color-brand)]">{timeRotation}°</span>
              </div>
              <input
                type="range"
                min="0"
