...
export function ArtifactCard({
  ...
  return (
    <div
      className={`group relative rounded-3xl border border-[var(--color-border)] bg-[#12121a] overflow-hidden flex flex-col hover:border-[var(--color-border)] transition-all duration-300 shadow-xl ${className}`}
    >
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
        <div>
          <span className="text-[10px] font-mono text-[var(--color-brand)] uppercase tracking-wider block">
            {civilization} · {dateOrEra}
          </span>
          <h4 className="text-xl font-bold text-white mt-1 group-hover:text-[var(--color-brand)] transition-colors">
            {title}
          </h4>
