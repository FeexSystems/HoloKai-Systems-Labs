...
export function CivilizationDossier({ civilization, onClose, className = '' }: CivilizationDossierProps) {
  return (
    <div className={`rounded-3xl border border-amber-500/30 bg-[#0a0a0f] p-8 md:p-12 space-y-8 text-white shadow-2xl backdrop-blur-2xl ${className}`}>
      {/* Dossier Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-amber-500/20 pb-6">
        <div>
          <div className="flex items-center gap-3">
            <span className="px-3 py-1 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-400 text-xs font-mono font-bold uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-4">
          <h3 className="text-lg font-bold text-amber-400 border-b border-white/10 pb-2">Historical Overview</h3>
          <p className="text-sm md:text-base text-zinc-200 leading-relaxed font-light">{civilization.description}</p>
        </div>

        <div className="space-y-4 p-6 rounded-2xl bg-[#12121a] border border-amber-500/20">
          <h4 className="text-xs font-mono uppercase text-amber-400 font-bold">Key Figures & Dynasties</h4>
