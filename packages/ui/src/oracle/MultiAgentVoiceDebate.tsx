...
const ALL_8_VANGUARDS: VanguardGuardian[] = [
...
export function MultiAgentVoiceDebate() {
  ...
  const startDebate = async () => {
    ...
    try {
      ...
      await oracleVoiceEngine.speakResponse(speakerA.openingLine);

      // Speaker B Turn
      if (isDebating) {
        setActiveSpeaker(speakerB.name);
        await oracleVoiceEngine.speakResponse(speakerB.openingLine);
      }
    } catch (err) {
    ...
    <div className="my-6 p-6 rounded-2xl bg-zinc-950/80 border border-amber-500/30 shadow-2xl backdrop-blur-xl">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-5 pb-3 border-b border-amber-500/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400">Pair any 2 Vanguard Guardians for a live spoken historical dialogue</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        <div className="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800 space-y-2">
          ...
          <p className="text-[11px] text-zinc-400 italic">"{speakerA.openingLine}"</p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800 space-y-2">
          ...
          <p className="text-[11px] text-zinc-400 italic">"{speakerB.openingLine}"</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        {activeSpeaker ? (
          ...
          <span className="text-xs text-zinc-400 font-mono">Ready to initiate spoken dialogue</span>
        )}
