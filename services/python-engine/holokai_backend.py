...
class CivilizationCore:
    def __init__(self, enable_vector_rag: bool | None = None):
        ...
        self.conversation_memory: ConversationMemory = get_conversation_memory()

    def process_query(self, query: str, session_id: Optional[str] = None) -> SynthesisResponse:
        ...
        if session_id:
            ...
            if context:
                logger.info(f"[{trace_id}] Retrieved context for session {session_id}")
        ...
        if session_id:
            self.conversation_memory.add_turn(
                ...
                active_agents=active_agents,
                confidence=confidence,
                metadata={"trace_id": trace_id, "plan_notes": plan.notes}
            )
